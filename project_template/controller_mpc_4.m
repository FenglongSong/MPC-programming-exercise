...
for k = 1:N-1
    ...
    constraints = [constraints, param.Xcons(:,1) - E{k} <= X{k+1} <= param.Xcons(:,2) + E{k}];
    constraints = [constraints, param.Ucons(:,1) <= U{k} <= param.Ucons(:,2)];
    constraints = [constraints, E{k} >= zeros(3,1)];
    objective = objective + U{k}'*R*U{k} + X{k}'*Q*X{k} + v*E{k} + E{k}'*S*E{k};
end

% get terminal cost
[~, P_inf, ~] = dlqr(param.A, param.B, Q, R);
objective = objective + X{N}'*P_inf*X{N} + v*E{N} + E{N}'*S*E{N};
